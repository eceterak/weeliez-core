...
<html lang="en">
	...
	<body>
		<div class = "container login-form form-container" id = "user-form">
			<?php if(!isset($error)): ?>
				<form action = "/user/res" name = "reset_password" class = "panel panel-shadow-uneven panel-content-lg" method = "POST" novalidate>
					<h4 class = "mb-3 action-header"><a href = "/">weeliez.com</a> / password reset</h4>
					<div class = "form-group">
						<label>New password</label>
						<input type = "password" name = "new_password" class = "form-control" />
					</div>
					<div class = "form-group">
						<label>Confirm password confirmation</label>
						<input type = "password" name = "confirm_password" class = "form-control" />
					</div>
					<?php if(isset($_SESSION['error'])): ?>
						<div class = "alert alert-danger dialog-message mt-2"><?php echo $_SESSION['error']; ?></div>
					<?php endif; ?>
					<p class = "small"><a href = "/user/login" class = "underline">Back to login</a></p>
					<input type = "hidden" name = "token" value = "<?php echo $token; ?>">
					<button type = "submit" class = "btn btn-primary btn-block">Submit</button>
				</form>
			<?php else: ?>
				<div class = "panel panel-shadow-uneven panel-content-lg">
					<h4 class = "mb-3 action-header">Error</h4>
					<p><?php echo $error; ?></p>
