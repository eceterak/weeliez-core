<?php
	...
	class mysqlib {
		...
		/**
		 * After completing a query, don't reset the _methods array with close() method.
		 * @see $this->close()
		 * @var bool
		 */

		private $reuseConditions = false;
		...
		/**
		 * This method works only on _temp array, its objective is to help create more advanced queries.
		 ...
		 * It's important to close connection before returning result. This is why there is only one return and return value is assigned to $rtn variable.
		...
		public function select($table, $columns = "*", $pagination = false) {
			if($pagination !== false) $pagination = $this->pagination($table, $pagination);
			...
			if($this->_subQuery === false) {
				...
				if($this->qsc === true) {
					...
					if($result->num_rows > 0) {
						...
						$rtn = new dataObject($arr, $pagination);
					}
					else $rtn = false; // This will return false.
					$this->close(); // Close connection and free results.
				}
			}
			else {
				$rtn = new dbObject('select');
				$rtn->setKey($this->_query);
				$rtn->setValue($this->_bindParams);
				$this->close();
			}
			return $rtn;
		}
		...
		public function selectOne($table, $columns = "*") {
			...
			if($this->qsc === true) {	
				$result = $this->stmt->get_result(); // Get results.
				if($result->num_rows > 0) {
					$rtn = $result->fetch_object(); // There is only one result. No need to run while or use array.
				}
				else {
					$rtn = false;
				}
		 ...
		 * Can use it to insert multiple records in one go. To do it, set $columns and use $this->values() method.
		 * @see values()
		 * @param $table [string]
		...
		public function insert($table, $columns = null) {
			$query = ($columns == null) ? $this->buildQuery("INSERT INTO ".$table, $this->_methods) : $this->buildQuery("INSERT INTO ".$table.' ('.$columns.')', $this->_methods);
			...
			if($this->qsc === true) {	
				...
				if($result->num_rows > 0) {
					...
					if($result->max == null) {
						...
						$rtn = $result->max; // Return only max number.
					}
				}
			}
			else $rtn = false;
			...
			if($this->qsc === true) {	
				...
				if($result->num_rows > 0) {
					...
					if($result->min == null) {
						...
						$rtn = $result->min; // Return only max number.
					}
				}
			}
			else $rtn = false;
		...
		public function minMax($table, $column, $columns = null) {
			...
			$this->query($this->_query); // Run Query.
				if($this->qsc === true) {
					...
					if($result->num_rows > 0) {
						while($res = $result->fetch_object()) {
							$rtn[] = $res; // Return array with data.
						}
					}
					else $rtn = false; // This will return false.
					$this->close(); // Close connection and free results.
		...
		public function delete($table) {
			...
			else $rtn = false; // There is no affected rows or no WHERE conditions sets. Nothing was deleted.
			$this->close();
			return $rtn;
		}
		...
		/**
		 * Use $values method to create multi insert query. 
		 * It's important to set values in the same order as in column names in a insert query.
		 * @see insert()
		 */

		public function values() {
			$values = func_get_args();
			$object = new dbObject('values');
			$object->setValue($values);
			$this->saveCondition('values', $object);
		}
		...
		public function buildQuery($preQuery, array $methods = null) {
			...
			if(isset($methods['values'])) $this->_query .= $this->buildValues($methods['values']);
		...
		public function buildWhere(array $methods, $start = true) {
			...
			foreach($methods as $key => $where) {
				...
				if(isset($where->methods)) {
					...
					if(is_object($where->key)) {
						switch($where->key->objectType()) {
							case 'select':
								$this->bindValues($where->key);
								$cond .= ' ('.$where->key->key.') '.$where->sign;
							break;
						}
					}
					else {
		 ...
		 * Each insert needs to be grouped in () and separated with a comma.
		 * @param methods [array]
		 * @return string
		 */

		public function buildValues(array $methods) {
			$cond = ' VALUES ';
			$keys = array_keys($methods);
			$end = end($keys);
			foreach($methods as $k => $values) {
				$this->bindValues($values);
				$cond .= ' (';
				$kz = array_keys($values->value);
				$last = end($kz);
				foreach($values->value as $key => $val) {
					$cond .= '? ';
					if($key != $last) $cond .= ', ';
				}
				$cond .= ')';
		 ...
		 * Get current page, count how many rows of data is available and divide it by desireable rows per page.
		 * Ceil function always rounds value up.
		 * @param $table [string]
		 * @param $perPage [int]
		 * @return string
		 */

		public function pagination($table, $perPage = 25) {
			$this->reuseConditions = true;
			$count = $this->count($table);
			if($count) {
				if(isset($_COOKIE['perpage'])) $perPage = $_COOKIE['perpage'];
